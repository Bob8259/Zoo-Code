...
describe("getOpenAiModels", () => {
	...
	describe("Gemini tool call backslash cleaning", () => {
		const systemPrompt = "You are a helpful assistant."
		const messages: Anthropic.Messages.MessageParam[] = [
			{
				role: "user",
		...
		it("should clean Gemini tool call backslashes in streaming mode", async () => {
			const geminiHandler = new OpenAiHandler({
				openAiApiKey: "test-api-key",
			...
			mockCreate.mockImplementation(async (options) => {
				return {
					[Symbol.asyncIterator]: async function* () {
						...
						// Yielding 4 backslashes: \\\\
						yield {
							choices: [
								{
									delta: {
										tool_calls: [{ index: 0, function: { arguments: '{"content":"\\\\\\\\' } }],
									},
									finish_reason: null,
								},
							],
			...
			const stream = geminiHandler.createMessage(systemPrompt, messages)
			const chunks: any[] = []
			for await (const chunk of stream) {
				chunks.push(chunk)
			}
			...
			const toolCallPartialChunks = chunks.filter((chunk) => chunk.type === "tool_call_partial")
			expect(toolCallPartialChunks).toHaveLength(3)
			// The second chunk had 4 backslashes in code, which should be expanded to 8 backslashes (\\\\\\\\ in string)
			expect(toolCallPartialChunks[1].arguments).toBe('{"content":"\\\\\\\\\\\\\\\\')
		})

		it("should handle split backslash chunks in streaming mode", async () => {
			const geminiHandler = new OpenAiHandler({
				openAiApiKey: "test-api-key",
			...
			mockCreate.mockImplementation(async (options) => {
				return {
					[Symbol.asyncIterator]: async function* () {
						...
						// Split \\\\ across two chunks: \\ in first, \\ in second
						yield {
							choices: [
								{
									delta: {
			...
			// Let's verify the total accumulated arguments on the consumer side is correctly expanded
			let accumulatedArgs = ""
			for (const chunk of toolCallPartialChunks) {
				accumulatedArgs += chunk.arguments || ""
			}
			expect(accumulatedArgs).toBe('{"content":"\\\\\\\\\\\\\\\\path"}')
		})

		it("should clean Gemini tool call backslashes in non-streaming mode", async () => {
			const geminiHandler = new OpenAiHandler({
				openAiApiKey: "test-api-key",
				openAiModelId: "gemini-1.5-flash",
				openAiBaseUrl: "https://api.openai.com/v1",
				openAiStreamingEnabled: false,
			})
			...
			const toolCallChunks = chunks.filter((chunk) => chunk.type === "tool_call")
			expect(toolCallChunks).toHaveLength(1)
			expect(toolCallChunks[0].arguments).toBe('{"content":"\\\\\\\\\\\\\\\\"}')
		})

		it("should NOT modify backslashes for non-Gemini models", async () => {
			...
			const stream = gptHandler.createMessage(systemPrompt, messages)
			const chunks: any[] = []
			for await (const chunk of stream) {
				chunks.push(chunk)
			}
