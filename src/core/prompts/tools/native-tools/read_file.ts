...
export function createReadFileTool(options: ReadFileToolOptions = {}): OpenAI.Chat.ChatCompletionTool {
	...
	const indentationProperties: Record<string, unknown> = {
		anchor_line: {
			type: "integer",
			description:
				"1-based line number to anchor the extraction (REQUIRED for indentation mode). Finds and completely returns the containing semantic block (function, class, etc.). Get this from search results, errors, or codebase_search.",
		},
		max_levels: {
			type: "integer",
			description: `Max indentation levels to include above the anchor (indentation mode, 0 = unlimited).`,
		},
		include_siblings: {
			type: "boolean",
			description: "Include sibling blocks at the same indentation level (indentation mode, default: false).",
		},
		include_header: {
			type: "boolean",
			description: "Include file header content like imports at the top (indentation mode, default: true).",
		},
		max_lines: {
			type: "integer",
	...
	const properties: Record<string, unknown> = {
		...
		offset: {
			type: "integer",
			description: "1-based line offset to start reading (slice mode, default: 1)",
		},
		limit: {
			type: "integer",
			description: `Max lines to return (slice mode, default: ${DEFAULT_LINE_LIMIT})`,
		},
		indentation: {
			type: "object",
			description: "Options for indentation mode. Only used when mode='indentation'. anchor_line is required.",
