...
// it should be consumed and used to fulfill the ask ONLY when type is completion_result.

describe("Task.ask queued message drain", () => {
	it("consumes queued message while blocked on completion_result ask", async () => {
		...
		const askPromise = task.ask("completion_result", "Q?", false)
	...
	it("does not consume queued messages for followup asks", async () => {
		const task = Object.create(Task.prototype) as Task
		;(task as any).abort = false
		;(task as any).clineMessages = []
		;(task as any).askResponse = undefined
		...
		expect((task as any).messageQueueService.messages[0]?.text).toBe("picked answer")
	})
